...
use crate::error::Result as BlsResult;
...
impl Generator {
    ...
    /// ```
    pub fn new() -> BlsResult<Generator> {
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<Generator> {
        Ok(Generator {
...
impl SignKey {
    ...
    ///
    /// ```
    /// use indy_blssignatures::SignKey;
    /// SignKey::new(None).unwrap();
    /// ```
    pub fn new(seed: Option<&[u8]>) -> BlsResult<SignKey> {
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<SignKey> {
        Ok(SignKey {
...
impl VerKey {
    ...
    /// ```
    pub fn new(gen: &Generator, sign_key: &SignKey) -> BlsResult<VerKey> {
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<VerKey> {
...
impl ProofOfPossession {
    ...
    ///
    /// ```
    /// use indy_blssignatures::{Generator, SignKey, VerKey, ProofOfPossession};
    /// let gen = Generator::new().unwrap();
    /// let sign_key = SignKey::new(None).unwrap();
    /// let ver_key = VerKey::new(&gen, &sign_key).unwrap();
    /// ProofOfPossession::new(&ver_key, &sign_key).unwrap();
    /// ```
    pub fn new(ver_key: &VerKey, sign_key: &SignKey) -> BlsResult<ProofOfPossession> {
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<ProofOfPossession> {
...
impl Signature {
    ...
    ///
    /// ```
    /// use indy_blssignatures::{SignKey, Signature, Bls};
    /// let sign_key = SignKey::new(None).unwrap();
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<Signature> {
        let point = PointG1::from_bytes(bytes)?;
        Ok(Signature {
            point,
            bytes: bytes.to_vec(),
...
impl MultiSignature {
    ...
    /// ```
    pub fn new(signatures: &[&Signature]) -> BlsResult<MultiSignature> {
    ...
    ///
    /// ```
    /// use indy_blssignatures::{SignKey, Bls, MultiSignature};
    /// let message = vec![1, 2, 3, 4, 5];
    /// let sign_key1 = SignKey::new(None).unwrap();
    /// let sign_key2 = SignKey::new(None).unwrap();
    /// let signature1 = Bls::sign(&message, &sign_key1).unwrap();
    ...
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> BlsResult<MultiSignature> {
        let point = PointG1::from_bytes(bytes)?;
        Ok(MultiSignature {
            point,
            bytes: bytes.to_vec(),
...
impl Bls {
    ...
    /// ```
    pub fn sign(message: &[u8], sign_key: &SignKey) -> BlsResult<Signature> {
    ...
    fn _gen_signature<T>(message: &[u8], sign_key: &SignKey, hasher: T) -> BlsResult<PointG1>
    where
        T: Digest,
    {
    ...
    pub(crate) fn _verify_signature<T>(
        signature: &PointG1,
        message: &[u8],
        ver_key: &PointG2,
        gen: &Generator,
    ...
    fn _hash<T>(message: &[u8], mut hasher: T) -> BlsResult<PointG1>
    where
        T: Digest,
    {
