...
///
/// Note: Generator instance deallocation must be performed by calling indy_bls_generator_free
///
/// # Arguments
/// * `gen_p` - Reference that will contain generator instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_generator_new(gen_p: *mut *const c_void) -> ErrorCode {
    trace!("indy_bls_generator_new: >>> gen_p: {:?}", gen_p);

    check_useful_c_ptr!(gen_p, 1);

    let res = match Generator::new() {
        Ok(gen) => {
            trace!("indy_bls_generator_new: gen: {:?}", gen);
            unsafe {
    ...
    trace!("indy_bls_generator_new: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_generator_from_bytes(
    ...
    trace!(
        "indy_bls_generator_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, gen_p: {:?}",
        bytes,
        bytes_len,
        gen_p
    );

    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(gen_p, 1);

    trace!("indy_bls_generator_from_bytes: bytes: {:?}", bytes);

    let res = match Generator::from_bytes(bytes) {
        Ok(gen) => {
            trace!("indy_bls_generator_from_bytes: gen: {:?}", gen);
            unsafe {
                *gen_p = Box::into_raw(Box::new(gen)) as *const c_void;
                trace!("indy_bls_generator_from_bytes: *gen_p: {:?}", *gen_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
    ...
    trace!("indy_bls_generator_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_generator_as_bytes(
    ...
    check_useful_c_reference!(gen, Generator, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    trace!("indy_bls_generator_as_bytes: >>> gen: {:?}", gen);
    ...
    trace!("indy_bls_generator_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `gen` - Generator instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_generator_free(gen: *const c_void) -> ErrorCode {
    trace!("indy_bls_generator_free: >>> gen: {:?}", gen);

    check_useful_c_ptr!(gen, 1);
    ...
    trace!("indy_bls_generator_free: <<< res: {:?}", res);
    res
}

/// Creates and returns random (or seeded from seed) BLS sign key algorithm requirements.
///
/// Note: Sign Key instance deallocation must be performed by calling indy_bls_sign_key_free.
///
/// # Arguments
...
pub extern "C" fn indy_bls_sign_key_new(
    ...
    trace!(
        "indy_bls_sign_key_new: >>> seed: {:?}, seed_len: {:?}, sign_key_p: {:?}",
        seed,
        seed_len,
        sign_key_p
    );

    check_useful_opt_c_byte_array!(seed, seed_len, 1, 2);

    trace!("indy_bls_sign_key_new: seed: {:?}", secret!(&seed));

    let res = match SignKey::new(seed) {
        Ok(sign_key) => {
            trace!("indy_bls_generator_new: gen: {:?}", secret!(&sign_key));
            unsafe {
                *sign_key_p = Box::into_raw(Box::new(sign_key)) as *const c_void;
                trace!("indy_bls_sign_key_new: *sign_key_p: {:?}", *sign_key_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
    ...
    trace!("indy_bls_sign_key_new: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_sign_key_from_bytes(
    ...
    trace!(
        "indy_bls_sign_key_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, gen_p: {:?}",
        bytes,
        bytes_len,
        sign_key_p
    );
    ...
    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(sign_key_p, 1);

    trace!("indy_bls_sign_key_from_bytes: bytes: {:?}", secret!(&bytes));

    let res = match SignKey::from_bytes(bytes) {
        Ok(sign_key) => {
            trace!(
                "indy_bls_sign_key_from_bytes: sign_key: {:?}",
                secret!(&sign_key)
            );
            unsafe {
                *sign_key_p = Box::into_raw(Box::new(sign_key)) as *const c_void;
                trace!(
                    "indy_bls_sign_key_from_bytes: *sign_key_p: {:?}",
                    *sign_key_p
                );
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_sign_key_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_sign_key_as_bytes(
    ...
    check_useful_c_reference!(sign_key, SignKey, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    trace!(
        "indy_bls_sign_key_as_bytes: sign_key: {:?}",
        secret!(sign_key)
    );
    ...
    trace!("indy_bls_sign_key_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `sign_key` - Sign key instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_sign_key_free(sign_key: *const c_void) -> ErrorCode {
    check_useful_c_ptr!(sign_key, 1);

    trace!(
        "indy_bls_sign_key_free: >>> sign_key: {:?}",
        secret!(sign_key)
    );
    ...
    trace!("indy_bls_sign_key_free: <<< res: {:?}", res);
    res
}

/// Creates and returns BLS ver key that corresponds to sign key.
///
/// Note: Verification key instance deallocation must be performed by calling indy_bls_ver_key_free.
///
/// # Arguments
...
#[no_mangle]
pub extern "C" fn indy_bls_ver_key_new(
    gen: *const c_void,
    sign_key: *const c_void,
    ver_key_p: *mut *const c_void,
) -> ErrorCode {
    trace!(
        "indy_bls_ver_key_new: >>> gen: {:?}, sign_key: {:?}, ver_key_p: {:?}",
        gen,
        sign_key,
        ver_key_p
    );
    ...
    check_useful_c_reference!(gen, Generator, 1);
    check_useful_c_reference!(sign_key, SignKey, 2);

    trace!(
        "indy_bls_ver_key_new: gen: {:?}, sign_key: {:?}",
        gen,
        secret!(sign_key)
    );
    ...
    let res = match VerKey::new(gen, sign_key) {
        Ok(ver_key) => {
            trace!("indy_bls_ver_key_new: ver_key: {:?}", ver_key);
            unsafe {
                *ver_key_p = Box::into_raw(Box::new(ver_key)) as *const c_void;
                trace!("indy_bls_ver_key_new: *ver_key_p: {:?}", *ver_key_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
...
///
/// Note: Verification key instance deallocation must be performed by calling indy_bls_very_key_free
///
/// # Arguments
/// * `bytes` - Bytes buffer pointer
/// * `bytes_len` - Bytes buffer len
...
pub extern "C" fn indy_bls_ver_key_from_bytes(
    ...
    trace!(
        "indy_bls_ver_key_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, gen_p: {:?}",
        bytes,
        bytes_len,
        ver_key_p
    );
    ...
    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(ver_key_p, 1);

    trace!("indy_bls_ver_key_from_bytes: bytes: {:?}", bytes);

    let res = match VerKey::from_bytes(bytes) {
        Ok(ver_key) => {
            trace!("indy_bls_ver_key_from_bytes: sign_key: {:?}", ver_key);
            unsafe {
                *ver_key_p = Box::into_raw(Box::new(ver_key)) as *const c_void;
                trace!("indy_bls_ver_key_from_bytes: *ver_key_p: {:?}", *ver_key_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
    ...
    trace!("indy_bls_ver_key_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_ver_key_as_bytes(
    ...
    check_useful_c_reference!(ver_key, VerKey, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    trace!("indy_bls_ver_key_as_bytes: ver_key: {:?}", ver_key);
    ...
    trace!("indy_bls_ver_key_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `ver_key` - Verification key instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_ver_key_free(ver_key: *const c_void) -> ErrorCode {
    check_useful_c_ptr!(ver_key, 1);

    trace!("indy_bls_ver_key_free: >>> ver_key: {:?}", ver_key);
    ...
    trace!("indy_bls_ver_key_free: <<< res: {:?}", res);
    res
}

/// Creates and returns BLS proof of possession that corresponds to ver key and sign key.
///
/// Note: Proof of possession instance deallocation must be performed by calling indy_bls_pop_free.
///
/// # Arguments
...
#[no_mangle]
pub extern "C" fn indy_bls_pop_new(
    ver_key: *const c_void,
    sign_key: *const c_void,
    pop_p: *mut *const c_void,
) -> ErrorCode {
    trace!(
        "indy_bls_pop_new: >>> ver_key: {:?}, sign_key: {:?}, pop_p: {:?}",
        ver_key,
        sign_key,
        pop_p
    );
    ...
    trace!(
        "indy_bls_pop_new: ver_key: {:?}, sign_key: {:?}",
        ver_key,
        sign_key
    );
    ...
    let res = match ProofOfPossession::new(ver_key, sign_key) {
        Ok(pop) => {
            trace!("indy_bls_pop_new: pop: {:?}", pop);
            unsafe {
                *pop_p = Box::into_raw(Box::new(pop)) as *const c_void;
                trace!("indy_bls_pop_new: *pop_p: {:?}", *pop_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
    ...
    trace!("indy_bls_pop_new: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_pop_from_bytes(
    ...
    trace!(
        "indy_bls_pop_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, gen_p: {:?}",
        bytes,
        bytes_len,
        pop_p
    );
    ...
    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(pop_p, 3);

    trace!("indy_bls_pop_from_bytes: bytes: {:?}", bytes);

    let res = match ProofOfPossession::from_bytes(bytes) {
        Ok(pop) => {
            trace!("indy_bls_pop_from_bytes: pop: {:?}", pop);
            unsafe {
                *pop_p = Box::into_raw(Box::new(pop)) as *const c_void;
                trace!("indy_bls_pop_from_bytes: *pop_p: {:?}", *pop_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
    ...
    trace!("indy_bls_pop_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_pop_as_bytes(
    ...
    check_useful_c_reference!(pop, ProofOfPossession, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    trace!("indy_bls_pop_as_bytes: pop: {:?}", pop);
    ...
    trace!("indy_bls_pop_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `pop` - Proof of possession instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_pop_free(pop: *const c_void) -> ErrorCode {
    check_useful_c_ptr!(pop, 1);

    trace!("indy_bls_pop_free: >>> pop: {:?}", pop);
    ...
    trace!("indy_bls_pop_free: <<< res: {:?}", res);
    res
}

/// Creates and returns signature from bytes representation.
///
/// Note: Signature instance deallocation must be performed by calling indy_bls_signature_free
///
/// # Arguments
/// * `bytes` - Bytes buffer pointer
/// * `bytes_len` - Bytes buffer len
...
pub extern "C" fn indy_bls_signature_from_bytes(
    ...
    trace!(
        "indy_bls_signature_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, signature_p: {:?}",
        bytes,
        bytes_len,
        signature_p
    );
    ...
    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(signature_p, 1);

    trace!("indy_bls_signature_from_bytes: bytes: {:?}", bytes);

    let res = match Signature::from_bytes(bytes) {
        Ok(signature) => {
            trace!("indy_bls_signature_from_bytes: signature: {:?}", signature);
            unsafe {
                *signature_p = Box::into_raw(Box::new(signature)) as *const c_void;
                trace!(
                    "indy_bls_signature_from_bytes: *signature_p: {:?}",
                    *signature_p
                );
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_signature_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_signature_as_bytes(
    ...
    check_useful_c_reference!(signature, Signature, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    trace!("indy_bls_signature_as_bytes: signature: {:?}", signature);
    ...
    trace!("indy_bls_signature_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `signature` - Signature instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_signature_free(signature: *const c_void) -> ErrorCode {
    check_useful_c_ptr!(signature, 1);

    trace!("indy_bls_signature_free: >>> signature: {:?}", signature);
    ...
    trace!("indy_bls_signature_free: <<< res: {:?}", res);
    res
}

/// Creates and returns multi signature for provided list of signatures.
///
/// Note: Multi signature instance deallocation must be performed by calling indy_bls_multi_signature_free.
///
/// # Arguments
...
pub extern "C" fn indy_bls_multi_signature_new(
    ...
    check_useful_c_reference_array!(signatures, signatures_len, Signature, 1, 2);
    check_useful_c_ptr!(multi_sig_p, 3);

    trace!("indy_bls_multi_signature_new: signatures: {:?}", signatures);

    let res = match MultiSignature::new(&signatures) {
        Ok(multi_sig) => {
            trace!("indy_bls_multi_signature_new: multi_sig: {:?}", multi_sig);
            unsafe {
                *multi_sig_p = Box::into_raw(Box::new(multi_sig)) as *const c_void;
                trace!(
                    "indy_bls_multi_signature_new: *multi_sig_p: {:?}",
                    *multi_sig_p
                );
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_multi_signature_new: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_multi_signature_from_bytes(
    ...
    trace!(
        "indy_bls_multi_signature_from_bytes: >>> bytes: {:?}, bytes_len: {:?}, multi_sig_p: {:?}",
        bytes,
        bytes_len,
        multi_sig_p
    );
    ...
    check_useful_c_byte_array!(bytes, bytes_len, 1, 2);
    check_useful_c_ptr!(multi_sig_p, 1);

    trace!("indy_bls_multi_signature_from_bytes: bytes: {:?}", bytes);

    let res = match MultiSignature::from_bytes(bytes) {
        Ok(multi_sig) => {
            trace!(
                "indy_bls_multi_signature_from_bytes: multi_sig: {:?}",
                multi_sig
            );
            unsafe {
                *multi_sig_p = Box::into_raw(Box::new(multi_sig)) as *const c_void;
                trace!(
                    "indy_bls_multi_signature_from_bytes: *multi_sig_p: {:?}",
                    *multi_sig_p
                );
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_multi_signature_from_bytes: <<< res: {:?}", res);
    res
}
...
pub extern "C" fn indy_bls_multi_signature_as_bytes(
    ...
    check_useful_c_ptr!(multi_sig, 1);
    check_useful_c_ptr!(bytes_p, 2);
    check_useful_c_ptr!(bytes_len_p, 3);

    let multi_sig = unsafe { &*(multi_sig as *const MultiSignature) };
    trace!(
        "indy_bls_multi_signature_as_bytes: multi_sig: {:?}",
        multi_sig
    );
    ...
    trace!("indy_bls_multi_signature_as_bytes: <<< res: {:?}", res);
    res
}
...
/// * `multi_sig` - Multi signature instance pointer
#[no_mangle]
pub extern "C" fn indy_bls_multi_signature_free(multi_sig: *const c_void) -> ErrorCode {
    check_useful_c_ptr!(multi_sig, 1);

    trace!(
        "indy_bls_multi_signature_free: >>> multi_sig: {:?}",
        multi_sig
    );
    ...
    trace!("indy_bls_multi_signature_free: <<< res: {:?}", res);
    res
}
...
/// deallocated by calling indy_bls_free_array.
///
/// # Arguments
///
...
pub extern "C" fn indy_bls_sign(
    ...
    trace!(
        "indy_bls_sign: >>> message: {:?}, message_len: {:?}, sign_key: {:?}, signature_p: {:?}",
        message,
        message_len,
        sign_key,
        signature_p
    ...
    check_useful_c_byte_array!(message, message_len, 1, 2);
    check_useful_c_reference!(sign_key, SignKey, 3);
    check_useful_c_ptr!(signature_p, 5);

    trace!(
        "indy_bls_sign: message: {:?}, sign_key: {:?}",
        message,
        secret!(sign_key)
    );
    ...
    let res = match Bls::sign(message, sign_key) {
        Ok(signature) => {
            unsafe {
                trace!("indy_bls_sign: signature: {:?}", signature);
                *signature_p = Box::into_raw(Box::new(signature)) as *const c_void;
                trace!("indy_bls_sign: *signature_p: {:?}", *signature_p);
            }
            ErrorCode::Success
        }
        Err(err) => fail(err),
...
pub extern "C" fn indy_bls_verify(
    ...
    trace!("indy_bls_verify: >>> signature: {:?}, message: {:?}, message_len: {:?}, ver_key: {:?}, gen: {:?}, valid_p: {:?}", signature, message, message_len, ver_key, gen, valid_p);

    check_useful_c_reference!(signature, Signature, 1);
    check_useful_c_byte_array!(message, message_len, 2, 3);
    check_useful_c_reference!(ver_key, VerKey, 4);
    check_useful_c_reference!(gen, Generator, 5);
    check_useful_c_ptr!(valid_p, 6);

    trace!(
        "indy_bls_verify: signature: {:?}, message: {:?}, ver_key: {:?}, gen: {:?}",
        signature,
        message,
        ver_key,
        gen
    ...
    let res = match Bls::verify(signature, message, ver_key, gen) {
        Ok(valid) => {
            trace!("indy_bls_verify: valid: {:?}", valid);
            unsafe {
                *valid_p = valid;
            }
            ErrorCode::Success
...
pub extern "C" fn indy_bls_verify_multi_sig(
    ...
    trace!("indy_bls_verify_multi_sig: >>> multi_sig: {:?}, message: {:?}, message_len: {:?}, ver_keys: {:?}, ver_keys_len: {:?}, gen: {:?}, valid_p: {:?}", multi_sig, message, message_len, ver_keys, ver_keys_len, gen, valid_p);

    check_useful_c_reference!(multi_sig, MultiSignature, 1);
    check_useful_c_byte_array!(message, message_len, 2, 3);
    check_useful_c_reference_array!(ver_keys, ver_keys_len, VerKey, 4, 5);
    check_useful_c_reference!(gen, Generator, 6);
    check_useful_c_ptr!(valid_p, 7);

    trace!(
        "indy_bls_verify_multi_sig: multi_sig: {:?}, message: {:?}, ver_keys: {:?}, gen: {:?}",
        multi_sig,
        message,
        ver_keys,
        gen
    ...
    let res = match Bls::verify_multi_sig(multi_sig, message, &ver_keys, gen) {
        Ok(valid) => {
            trace!("indy_bls_verify_multi_sig: valid: {:?}", valid);
            unsafe {
                *valid_p = valid;
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_verify_multi_sig: <<< res: {:?}", res);
    res
}
...
/// * `valid_p` - Reference that will be filled with true - if signature valid or false otherwise.
#[no_mangle]
pub extern "C" fn indy_bls_verify_pop(
    pop: *const c_void,
    ver_key: *const c_void,
    gen: *const c_void,
    valid_p: *mut bool,
    ...
    trace!(
        "indy_bls_verify_pop: >>> pop: {:?}, ver_key: {:?}, gen: {:?}, valid_p: {:?}",
        pop,
        ver_key,
        gen,
        valid_p
    ...
    check_useful_c_reference!(pop, ProofOfPossession, 1);
    check_useful_c_reference!(ver_key, VerKey, 2);
    check_useful_c_reference!(gen, Generator, 3);
    check_useful_c_ptr!(valid_p, 4);

    trace!(
        "indy_bls_verify_pop: pop: {:?}, ver_key: {:?}, gen: {:?}",
        pop,
        ver_key,
        gen
    );
    ...
    let res = match Bls::verify_proof_of_posession(pop, ver_key, gen) {
        Ok(valid) => {
            trace!("indy_bls_verify_pop: valid: {:?}", valid);
            unsafe {
                *valid_p = valid;
            }
            ErrorCode::Success
    ...
    trace!("indy_bls_verify_pop: <<< res: {:?}", res);
    res
}
...
mod tests {
    ...
    #[test]
    fn indy_bls_generator_new_works() {
        let mut gen: *const c_void = ptr::null();

        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        assert!(!gen.is_null());

        let err_code = indy_bls_generator_free(gen);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_generator_as_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    #[test]
    fn indy_bls_generator_from_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_generator_from_bytes(bytes, bytes_len, &mut gen2);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_generator_free(gen);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_generator_free(gen2);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_generator_free_works() {
        let mut gen: *const c_void = ptr::null();
    ...
    fn indy_bls_sign_key_new_works() {
        ...
        let err_code = indy_bls_sign_key_free(sign_key);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_sign_key_new_works_for_seed() {
        let mut sign_key: *const c_void = ptr::null();
    ...
    fn indy_bls_sign_key_from_bytes_works() {
        ...
        let err_code = indy_bls_sign_key_from_bytes(bytes, bytes_len, &mut sign_key2);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_sign_key_free(sign_key);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_sign_key_free(sign_key2);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    fn indy_bls_ver_key_new_works() {
        ...
        let err_code = indy_bls_ver_key_new(gen, sign_key, &mut ver_key);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_ver_key_free(ver_key);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_ver_key_as_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    #[test]
    fn indy_bls_ver_key_from_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_ver_key_from_bytes(bytes, bytes_len, &mut ver_key2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_ver_key_free(ver_key2);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_ver_key_free_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_bls_signature_as_bytes_works() {
        ...
        let err_code = indy_bls_signature_free(signature);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    fn indy_bls_signature_from_bytes_works() {
        ...
        let err_code = indy_bls_signature_from_bytes(bytes, bytes_len, &mut signature2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_signature_free(signature2);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    fn indy_bls_multi_signature_new_works() {
        ...
        let mut signature2: *const c_void = ptr::null();
        let err_code = indy_bls_sign(message, message_len, sign_key2, &mut signature2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_signature_free(signature1);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_signature_free(signature2);
        assert_eq!(err_code, ErrorCode::Success);

        let err_code = indy_bls_multi_signature_free(multi_sig);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    fn indy_bls_multi_signature_from_bytes_works() {
        ...
        let err_code = indy_bls_multi_signature_from_bytes(bytes, bytes_len, &mut multi_sig2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_multi_signature_free(multi_sig2);
        assert_eq!(err_code, ErrorCode::Success);
    }
    ...
    fn indy_bls_verify_works() {
        ...
        let err_code = indy_bls_verify(signature, message, message_len, ver_key, gen, &mut valid);
        assert_eq!(err_code, ErrorCode::Success);
        assert!(valid);
    ...
    #[test]
    fn indy_bls_verify_works_for_invalid() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let mut ver_key: *const c_void = ptr::null();
        let err_code = indy_bls_ver_key_new(gen, sign_key2, &mut ver_key);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_bls_pop_new_works() {
        ...
        let err_code = indy_bls_pop_new(ver_key, sign_key, &mut pop);
        assert_eq!(err_code, ErrorCode::Success);
        assert!(!pop.is_null());
        ...
        let err_code = indy_bls_pop_free(pop);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_pop_as_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    #[test]
    fn indy_bls_pop_from_bytes_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_pop_from_bytes(bytes, bytes_len, &mut pop2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_pop_free(pop2);
        assert_eq!(err_code, ErrorCode::Success);
    }

    #[test]
    fn indy_bls_pop_free_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    fn indy_bls_verify_pop_works() {
        ...
        let err_code = indy_bls_verify_pop(pop, ver_key, gen, &mut valid);
        assert_eq!(err_code, ErrorCode::Success);
        assert!(valid);
    ...
    #[test]
    fn indy_bls_verify_multi_sig_works() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_ver_key_new(gen, sign_key1, &mut ver_key1);
        assert_eq!(err_code, ErrorCode::Success);

        let mut ver_key2: *const c_void = ptr::null();
        let err_code = indy_bls_ver_key_new(gen, sign_key2, &mut ver_key2);
        assert_eq!(err_code, ErrorCode::Success);

        let ver_keys = [ver_key1, ver_key2];
        let mut valid = false;

        let err_code = indy_bls_verify_multi_sig(
            multi_sig,
            message,
            message_len,
            ver_keys.as_ptr(),
        ...
        let err_code = indy_bls_ver_key_free(ver_key1);
        assert_eq!(err_code, ErrorCode::Success);
    ...
    #[test]
    fn indy_bls_verify_multi_sig_works_for_invalid() {
        let mut gen: *const c_void = ptr::null();
        let err_code = indy_bls_generator_new(&mut gen);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let mut ver_key2: *const c_void = ptr::null();
        let err_code = indy_bls_ver_key_new(gen, sign_key3, &mut ver_key2);
        assert_eq!(err_code, ErrorCode::Success);
        ...
        let err_code = indy_bls_sign_key_free(sign_key3);
        assert_eq!(err_code, ErrorCode::Success);
